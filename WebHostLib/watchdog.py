...
class WebHostEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if event.src_path == self.base_path:
            return    
        print(f"[WATCHDOG] Datei geändert: {event.src_path}")
    ...
    def on_created(self, event):
        ...
        if not self._is_direct_child(event.src_path):
            return
        print(f"[WATCHDOG] Datei erstellt: {event.src_path}")
    ...
    def on_deleted(self, event):
        ...
        if not self._is_direct_child(event.src_path):
            return
        print(f"[WATCHDOG] Datei gelöscht: {event.src_path}")
...
class FolderWatchdog:
    ...
    def start(self):
        ...
        self.observer.start()
        print(f"[WATCHDOG] Beobachte {self.path}")
